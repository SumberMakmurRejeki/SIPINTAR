    ...
    <div class="space-y-6">
        ...
        <div class="grid gap-4 sm:grid-cols-3">
            <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
                ...
                <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $test->duration_minutes ? $test->duration_minutes . ' menit' : '-' }}</p>
            </div>
            <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
                <p class="text-sm text-gray-500 dark:text-gray-400">Passing Grade</p>
                <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $test->passing_grade !== null ? $test->passing_grade : '-' }}</p>
            </div>
            <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
                <p class="text-sm text-gray-500 dark:text-gray-400">Batas Percobaan</p>
                <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $test->max_attempts !== null ? $test->max_attempts : '-' }}</p>
            </div>
        </div>
        ...
        <div class="space-y-4">
            ...
            @empty
                <div class="rounded-lg border border-dashed border-gray-300 p-6 text-center text-gray-500 dark:border-gray-700">
                    Belum ada pertanyaan post-test.
                </div>
            @endforelse
        </div>
    </div>
